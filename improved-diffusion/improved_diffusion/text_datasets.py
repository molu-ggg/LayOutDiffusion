...
def helper_tokenize_encode(sentence_lst, vocab_dict, model, seqlen, data_args, padding_mode, ):
    result_train_lst = [] ###-  对输入的句子列表进行分词和编码，并生成相应的隐藏状态。
    group_lst = defaultdict(list)
    with torch.no_grad():
...
#                         word_lst = [x.text for x in tokenizer(word_lst)]   ###- 我给替换了
                        word_lst = [x for x in word_lst.split(" ")]
    ...
    # print("vocab_dict",path_save_vocab,vocab_dict)
    ...
    if not os.path.exists(path_save) and data_args.experiment == 'random':
        torch.save(model.state_dict(), path_save)  ###- 保存模型
