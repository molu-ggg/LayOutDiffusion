...
def load_models(modality, mode, model_name_or_path, emb_dim, file, extra_args=None):
    if mode in ["random", "random1", "random_up_proj", "glove"]:
        if modality == "book" or (
            extra_args is not None and extra_args.use_bert_tokenizer == "yes"
        ):
...
def rounding_func(mode, text_emb_lst, model, tokenizer, emb_scale_factor=1.0):
    ...
    if mode in ["random", "random_up_proj", "glove"]:
        ...
        def get_knn(down_proj_emb, text_emb, dist="cos"):
            if dist == "cos":
                adjacency = down_proj_emb @ text_emb.transpose(1, 0).to(
                    down_proj_emb.device
                )
            elif dist == "l2":
                adjacency = down_proj_emb.unsqueeze(1).expand(
                    -1, text_emb.size(0), -1
                ) - text_emb.unsqueeze(0).expand(down_proj_emb.size(0), -1, -1)
        ...
        for text_emb in text_emb_lst:
            ...
            if len(text_emb.shape) > 2:
                ...
                text_emb = text_emb
            val, indices = get_knn(
                (down_proj_emb2 if dist == "cos" else down_proj_emb),
