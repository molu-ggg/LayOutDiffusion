...
def compute_maximum_iou(layouts_1, layouts_2, n_jobs=None):
    ...
    print(f"gt label set: {len(c2bl_1.keys())!s}, pred label set: {len(c2bl_2.keys())!s}, intersection set: {len(keys)!s}")
...
def __compute_mean_iou_for_layout(layout_1, layout_2):
    score = 0.0
    (bi, li), (bj, lj) = layout_1, layout_2
    N = len(bi)
    for l in list(set(li.tolist())):
...
def __compute_mean_iou(layouts_1_and_2):
    layouts_1, layouts_2 = layouts_1_and_2
    N, M = len(layouts_1), len(layouts_2)
    ii, jj = np.meshgrid(range(N), range(M))
    ii, jj = ii.flatten(), jj.flatten()
    scores = np.asarray(
        [
            __compute_mean_iou_for_layout(layouts_1[i], layouts_2[j])
            for i, j in zip(ii, jj)
        ]
    ).reshape(N, M)
    return (scores.sum() - N) / (1 * (N - 1))


def compute_self_sim(layouts_1, layouts_2, n_jobs=50):
    layouts_1 = layouts_2
    c2bl_1 = __get_cond2layouts(layouts_1)
    keys_1 = set(c2bl_1.keys())
    c2bl_2 = __get_cond2layouts(layouts_2)
    ...
    for key in keys:
        if len(c2bl_1[key]) > 1:
            args.append((c2bl_1[key], c2bl_2[key]))
    print("label set with one or more: ", len(args))
    with mp.Pool(n_jobs) as p:
        scores = p.map(__compute_mean_iou, args)
    scores = np.asarray(scores)
    return scores.sum().item() / len(layouts_1)
